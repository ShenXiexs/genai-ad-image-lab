...
定位：**以 Park et al. (1986) 的 brand concept management 为策略母体，把 functional、experiential 与 symbolic brand meaning 转译为可控制、可审计的 GenAI 广告图片生成策略，再检验普通用户能否按设计者意图读出这些意义，以及来源披露和产品情境如何改变这种意义传达及其后续决策结果。**
...
| 本研究生图策略                                    | Park et al. (1986) 的概念内核                                                                                                                              | 原文对定位／传播的含义                                                                                             | 本研究对 GenAI 广告图的转译                                                                                                   |
| ------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------- |
| 功能型／产品导向（Functional / Product-oriented） | functional needs 驱动消费者寻找能够解决消费相关问题的产品；functional concept 面向外部产生的消费需要                                                       | 在导入阶段应强调品牌解决消费问题的 functional performance（pp. 136, 140）                                          | 让商品及其可诊断的功能、使用方式或问题解决结果成为主导信息；不能仅用白底产品照代替功能意义                                    |
| 体验型（Experiential-oriented）                   | experiential needs 包括感官愉悦、变化以及认知刺激；experiential concept 满足内部产生的刺激或变化需要                                                       | 应传达 sensory satisfaction 或 cognitive stimulation，并突出消费中的 experiential／fantasy aspects（pp. 136, 140） | 通过使用情境、感官结果和可想象的消费过程，使受众能够模拟“使用它是什么体验”；不能把 aesthetic quality 本身当作体验意义       |
| 象征型（Symbolic-oriented）                       | symbolic needs 涉及 self-enhancement、role position、group membership 与 ego-identification；symbolic concept 将个体与期望群体、角色或 self-image 联系起来 | 应突出品牌与 group membership 或 self-identification 的关系（pp. 136, 140）                                        | 通过社会角色、参照群体、身份表达或理想自我线索建立 product–self connection；不能把 symbolic 简化为“昂贵”“奢华”或高端滤镜 |
...
选择广告图片不是因为新闻不重要，而是因为本研究的问题属于另一类 HCI 设计空间。新闻研究关注 LLM 如何改变既有文本的 tone、framing、bias 和 credibility；本研究关注 T2I 系统如何把同一个商品转译为不同的视觉价值主张，以及消费者能否恢复这一设计意图。广告图片具有三个方法优势：其一，白底商品参考图提供稳定的视觉锚点，可以在保持商品身份不变的前提下改变传播策略；其二，Park et al. (1986) 明确把 functional、symbolic、experiential 视为可以由管理活动创造的品牌形象而非固定产品类别，这为同一商品的三策略生成提供了理论依据；Homburg et al. (2015) 和 Brakus et al. (2009) 则帮助补充后续操作化，避免用“cinematic”“luxury”“minimalist”等纯风格词替代理论构念；其三，广告图同时连接生成界面、视觉理解和行为评价，使研究能够观察从生成控制到意义解码，再到广告态度和购买意愿的完整链条。
...
### 2.3 商品与图片最初各选多少：当前工作流程与正式方案目标

| 层级 | 数量 | 含义与状态 |
| --- | ---: | --- |
| CSV 原始商品池 | 120 个 | 根据研究者当前记录，完整筛选池包含 120 个商品；这一层只表示材料来源，不表示商品已经符合四格分类。 |
| 研究团队初筛短名单 | 24 个 | 在此前约 20 个工作候选的基础上，按照白底源图清晰度、商品主体可保持性、类别重复度以及四个属性格的覆盖情况，补充并正式形成 24 个待评商品。正式方法报告以 24 个为准。 |
| 专家筛选后的正式商品池 | 12 个 | 专家评分结合模型辅助复核后，从 24 个商品中保留 12 个；目标为实用／享乐 × 高／低象征性四格各 3 个。若某格不足，不为追求平衡而强留不合格商品。 |
| 每个商品 × 策略的图片候选 | 至少 3 张 | 12 个商品分别生成 functional、experiential、symbolic 三类图片；现有多轮生成结果已提供多个候选，下一步主要是整理、编号和验证候选。 |
| 进入 manipulation check 的候选图 | 至少 108 张 | `12 商品 × 3 策略 × 每单元至少 3 张候选`，即每种策略至少 36 张候选。 |
| 正式冻结刺激 | 36 张 | manipulation check 后，每个`商品 × 策略`保留 1 张，即 `12 × 3 × 1`；三种策略各 12 张。 |

> **数量口径说明。** `120` 是研究者确认的完整商品筛选池数量；当前仓库中的 `data/experiment/Experiment-Test0709.csv` 仍是包含 15 个商品的早期子集。正式报告或预注册前，应将 120 个商品对应的 CSV 或 selection manifest 另行保存并锁定版本，使“120 → 24 → 12”的筛选链可以由文件复核。

### 2.4 本研究选图步骤与每一步的理由

| 步骤 | 操作 | 选择／排除规则 | 理由 |
| --- | --- | --- | --- |
| 1. 建立并初筛商品候选池 | 以完整 CSV 中的 120 个商品为原始池，先按白底参考图质量、商品主体可保持性、类别重复度以及实用／享乐 × 高／低象征性四格覆盖进行研究团队初筛，形成 24 个待评商品。 | 排除源图不清、主体边界含混、生成时难以保持商品身份或类别高度重复的商品；24 个是进入正式专家评分的短名单，而不是最终实验商品。 | 先控制源商品的技术可用性和属性覆盖，避免生成阶段把源图缺陷、商品类别差异或四格失衡误认为策略效应。 |
| 2. 专家商品筛选与模型辅助复核 | 由专家组独立评价 24 个商品的 utilitarian orientation、hedonic orientation 与 symbolic affordance，并评价熟悉度和预期价格等控制维度；另用 GPT 等模型按同一 rubric 独立评分，作为一致性复核和分歧诊断。 | 最终由人类专家评分主导，从 24 个商品中筛选 12 个，目标为四格各 3 个；模型评分与专家评分分别报告，模型不参与人类熟悉度和预期价格的最终判定，也不以模型多数票替代专家决定。具体保留阈值与分歧处理规则在筛选前确定。 | 借鉴 Khatiwada et al. (2026) 的多模型交叉复核思路，同时保持构念效度：模型可以帮助发现分类不稳定或 rubric 歧义，商品的正式纳入仍建立在人类判断上。 |
| 3. 受控候选生成与整理 | 对最终 12 个商品分别实施 functional、experiential、symbolic 三种策略，每个`商品 × 策略`整理至少 3 张候选图；现有多轮生成实验已基本完成候选积累。 | 形成 `12 × 3 × ≥3` 的候选集合。固定模型、wrapper、1:1 图幅、源图和共同禁则；策略 prompt 分别突出问题解决、感官／认知体验、群体／角色／自我形象，只改变主导意义路线。统一记录 prompt、版本、seed、生成轮次和候选编号。 | 让条件差异尽可能归因于理论定义的视觉意义策略，而不是模型版本、表面 style modifier 或未记录的反复试生成。 |
| 4. Manipulation check 与正式刺激选择 | 向独立人类样本隐藏策略标签，对每个`商品 × 策略`的候选图测量 functional／experiential／symbolic 主导意义、产品保真、视觉质量以及广告连贯性／可信度；据此从每个单元的至少 3 张候选中选定 1 张。若某单元没有合格候选，仅在该单元内替换或补充图片并重新接受同一 check，不再单列“定向替换”步骤。 | 最终阈值在实施前预注册。可讨论的候选标准包括：目标意义识别率及其相对 33.3% 猜测基线的置信区间、产品保真与视觉质量均值，以及同一商品三种策略之间的质量差异；当前的 50%、4.5/7 和 0.60 仅作为待讨论示例，不写成已经确定的规则。不得依据广告态度、购买意愿、传播意愿或主实验结果反向选图。 | 同时验证意义操纵是否成功并控制基础画质，确保受众读出的是问题解决、体验刺激或身份／群体意义。失败单元在同一 manipulation-check 机制内修复，可以避免 LLM 自评替代人评，也避免利用核心 DV 优化刺激。 |
| 5. 冻结与审计 | 冻结 `12 × 3 = 36` 张正式图片及 stimulus manifest，保留源图、prompt、模型与 wrapper 版本、seed、候选历史、评分结果和替换记录。 | 每个`商品 × 策略`固定 1 张；正式 Study 1 开始后不得更换刺激。 | 保证实验可复现，并使成功刺激、失败模式和修复轨迹都能够转化为 HCI 设计知识。 |

这一流程把 CHI 新闻研究的核心原则推广到广告图片：**120 个原始商品 → 24 个研究团队初筛商品 → 12 个专家筛选商品 → 每个商品的三策略候选图 → manipulation check → 36 张冻结刺激。** GPT 等模型只用于商品层的辅助复核，不替代专家筛选；图片层则必须由独立人类样本完成 manipulation check。该流程进一步处理了图片域特有的商品身份保持、视觉 artifact、虚构功能、跨策略画质平衡和商业意义识别问题。

## 3. 核心 HCI 论文如何组织 Background / Related Work
...
| Otani et al. (2023), CVPR，全文核验                          | Introduction：自动指标与人类感知错位 → 审计 37 篇论文发现协议和报告不一致；Related Work：human evaluation → crowdsourcing quality → automatic metrics                                                                                                           | 不只是“需要人评”，而是需要可复现、标准化、能报告标注质量的人评协议                                                                                  | 刺激层 manipulation check 应把视觉质量、产品保真、意义保真和总体偏好分开；报告每张图的评价人数、呈现方式、量尺、随机化与一致性                                      |
...
| Corneanu et al. (2025), GenomeBench | scene graph + 引导式人类问答，分解整体含混判断                              | 刺激层 manipulation check 的题项应具体到产品、场景、属性和关系，减少“总体上是否匹配”的歧义 |
...
| Park et al. (1986)，本地全文核验            | brand concept 是由企业选择、源于消费者需要并指导 positioning 与 brand image 的上位意义；functional 对应消费问题解决，experiential 对应感官愉悦／变化／认知刺激，symbolic 对应群体、角色与 self-image；三者指向可被创造的 image 而非固定 product class（pp. 135–140） | 本研究三类生图策略的首要理论依据：同一商品可以承载三种不同主导形象；但原文不是 T2I 研究或图片量表，因此必须把理论定义转成 prompt、排除规则和受众意义验证 |
...
Park et al. (1986) 已经提供了清晰的策略逻辑：品牌管理者先从消费者需要中选择 functional、symbolic 或 experiential concept，再用与之匹配的 positioning activities 建立 brand image；更关键的是，这三类概念指的是被创造的 image，而不是固定 product class（p. 136）。视觉修辞及后续消费者研究则说明图片能够承载这些意义，并可能触发诊断性、心理模拟和自我联结（Scott, 1994; Jiang & Benbasat, 2007; Elder & Krishna, 2012; Homburg et al., 2015）。然而，Park et al. 没有把框架操作化为图片生成协议或受众量表，传统广告研究也通常使用不同广告、品牌或人工制作刺激，难以把产品身份、画质、构图和意义策略同时控制。

GenAI 提供了一个新的实验能力：把 Park 的三种 positioning logics 写成不同的生成约束，在同一去品牌产品和相同技术条件下系统地产生多种主导意义。因此，本研究的理论推进不是重新发明三分类，而是把 **brand concept selection → positioning strategy → perceived image** 转译为 **theoretical concept → generation prompt／curation rule → audience-perceived visual meaning**。这种转译只有在经过产品保真、画质和意义理解的多阶段验证后，才能成为有效的因果操作。
...
### 7.3 Functional, Experiential, and Symbolic Meaning in Product Images

本节应把 Park et al. (1986) 放在首位并展开，而不是只用一句话列为三分类来源。写作顺序建议为：① brand concept 是由企业选择、源于消费者需要并指导 positioning 与 image 的上位意义；② functional 是问题解决，experiential 是感官／认知刺激，symbolic 是群体、角色与 self-image；③ 三者描述的是可被管理活动创造的 image，而不是固定 product class，因此同一商品三策略生成具有理论合法性；④ Park 的框架不是图片量表，需要 Homburg et al. (2015)、Candi et al. (2017) 和 Brakus et al. (2009) 帮助操作化，再分别连接 diagnosticity、mental simulation 和 self-connection（Jiang & Benbasat, 2007; Elder & Krishna, 2012; Escalas & Bettman, 2003）。结尾引出产品实用／享乐取向和象征承载力是匹配条件，而不是三类策略本身或全局排名。

### 7.4 AI Disclosure as a Source Cue in Visual Communication
...
### 7.5 Research Questions and Design Contribution
...
1. **把 Park et al. (1986) 写入生成协议本身。** 每类 prompt、正向线索、禁则和 meaning-check 题项都应能回溯到问题解决、感官／认知刺激、群体／角色／self-image 三类定义；不能只在 Introduction 引用一次。
2. **区分商品筛选与图片 manipulation check。** 商品的实用／享乐取向和象征承载力先用于“24 → 12”的专家筛选；图片质量、产品保真和意义保真随后在一个刺激层 manipulation check 中统一验证。二者都不是 AI 披露前测。
3. **将披露固定为 Study 1 的并列组间因素。** 参与者在首次看图前一次性随机进入 AI 已披露组或来源未披露组；同一参与者不跨组、不在披露前后重复评价，也不另设披露 Study 2。
4. **不要把无披露组写成人类制作。** 使用“未说明来源”，否则实验比较的是 AI vs. human source claim，而不是 disclosure vs. no source disclosure。
5. **把意义识别放在独立 manipulation check 中，不在正式研究中反复追问。** 正式研究逐图问“这是不是 AI”或“它是哪类策略”会显著启动研究目的和 persuasion knowledge。
6. **所有核心 DV 使用 post-only 比较。** 满意／广告态度、购买、推荐、分享、可信度和真实性等结果均在参与者所属条件下测量一次；不计算 disclosure pre–post change score。
7. **只把来源推断和披露回忆作为后置检查。** 仅在来源未披露组完成所有核心 DV 后测一次 inferred AI authorship；两组在结果题完成后回答 disclosure recall。二者不属于披露前测，也不用于 ITT 排除。
8. **分离 aesthetic、experiential 与 authenticity。** 美观是刺激质量；体验意义是图片让使用体验变得可感；真实性是对内容／品牌是否真实可信的判断。
9. **随机效应覆盖 participant、product 和 image。** 如果每个产品只有一张策略图，策略效应会与图片实例混淆；最好每个 product × strategy 有多个合格实例，或至少在限制中明确刺激抽样边界。
10. **报告失败刺激。** HCI 贡献不仅是最后选中的“成功图片”，还包括不同策略常见的失败模式，例如功能信息不足、体验场景抢走产品主体、象征线索含混或身份意义被误读。
11. **将 2026 最近邻写入论文。** Shi and Jiang (2026)、Gu et al. (2026)、Pawelczyk et al. (2026) 和 Trattner et al. (2026) 会让背景更及时，也能迫使贡献陈述保持精确。
