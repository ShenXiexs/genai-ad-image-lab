...
| 步骤                | 操作                                                                             | 选择／排除规则                                                                                                                          | 理由                                                                                              |
| ------------------- | -------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| 1. 建立商品候选池   | 从具有清晰白底参考图的商品中建立实用／享乐 × 高／低象征性四格候选               | 去除商品主体不清或类别重复严重的材料                                                                                                    | 先保证源商品可控，避免生成阶段无法区分策略效应与源图缺陷                                          |
| 2. 商品层预检验     | 由独立中国成年样本评价实用性、享乐性、象征承载力、熟悉度和预期价格               | 每格至少保留 3 个；目标取向差异和象征性差异达到预注册阈值；控制极端熟悉度／价格                                                         | 商品分类必须来自受众判断，而不是研究者标签                                                        |
| 3. 受控候选生成     | 对每个保留商品分别生成三种策略，每个`商品 × 策略` 至少 3 张                   | 固定模型、wrapper、1:1 图幅、源图和共同禁则；策略 prompt 分别围绕问题解决、感官／认知体验、群体／角色／自我形象线索，只改变主导意义路线 | 让条件差异尽可能归因于理论定义的视觉意义策略，而非表面 style modifier                             |
| 4. 独立人类刺激验证 | 不显示策略标签，按照三类需要定义测主导意义三选一，并测产品保真和广告连贯／可信度 | *目标路线选择率至少 50%，其 95% CI 排除 33.3% 猜测；产品保真与视觉质量均值至少 4.5/7；同商品三策略质量差不超过 0.60*                  | 检验受众读出的是否是问题解决、体验刺激或身份／群体意义；不能用 prompt 名称或 LLM 自评代替构念实现 |
| 5. 定向替换         | 只修改失败的`商品 × 策略` prompt，并用独立样本复核替换图                      | 不回看广告态度、购买意愿或主实验结果选图                                                                                                | 把修复限制在失败单元，避免利用结果变量反向优化刺激                                                |
| 6. 冻结与审计       | 冻结 36 张正式图及 manifest，保留prompt、版本、seed                              | 正式 Study 1 启动后不得换图                                                                                                             | 保证可复现性，并把成功与失败模式都转化为 HCI 设计知识                                             |
...
2. **保留 Study 0A 与 0B。** 产品取向和象征承载力先在产品层验证；图片质量、产品保真和意义保真在刺激层验证。它们是产品／刺激验证，不是 AI 披露前测。
...
5. **把意义识别放在独立刺激验证中，不在正式研究中反复追问。** 正式研究逐图问“这是不是 AI”或“它是哪类策略”会显著启动研究目的和 persuasion knowledge。
